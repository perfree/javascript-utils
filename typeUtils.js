/**
 * 类型判断工具
 * @author Perfree
 */
var typeUtils = {
	
	/**
	 * 是否为字符串
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isString(obj)
	 */
	isString: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'String'
	},

	/**
	 * 是否为数字
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isNumber(obj)
	 */
	isNumber: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Number'
	},

	/**
	 * 是否为boolean
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isBoolean(obj)
	 */
	isBoolean: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Boolean'
	},

	/**
	 * 是否是函数
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isFunction(obj)
	 */
	isFunction: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Function'
	},

	/**
	 * 是否为空
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isNull(obj)
	 */
	isNull: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Null'
	},

	/**
	 * 是否为undefined
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isUndefined(obj)
	 */
	isUndefined: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Undefined'
	},

	/**
	 * 是否为对象Object
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isObj(obj)
	 */
	isObj: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Object'
	},

	/**
	 * 是否为数组
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isArray(obj)
	 */
	isArray: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Array'
	},

	/**
	 * 是否为时间类型
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isDate(obj)
	 */
	isDate: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Date'
	},

	/**
	 * 是否为正则
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isRegExp(obj)
	 */
	isRegExp: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'RegExp'
	},

	/**
	 * 是否为错误对象Error
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isError(obj)
	 */
	isError: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Error'
	},

	/**
	 * 是否为Symbol函数
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isSymbol(obj)
	 */
	isSymbol: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Symbol'
	},

	/**
	 * 是否为Promise对象
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isPromise(obj)
	 */
	isPromise: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Promise'
	},

	/**
	 * 是否为set对象
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isSet(obj)
	 */
	isSet: function(obj) {
		return Object.prototype.toString.call(obj).slice(8, -1) === 'Set'
	},

	/**
	 * 是否为False
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isFalse(obj)
	 */
	isFalse: function(obj) {
		if (!obj || obj === 'null' || obj === 'undefined' || obj === 'false' || obj === 'NaN') return true
		return false
	},

	/**
	 * 是否为true
	 * @param  {obj} 要判断的对象
	 * @return {Boolean} 布尔
	 * @example typeUtils.isTrue(obj)
	 */
	isTrue: function(obj) {
		return !this.isFalse(obj)
	},

	/**
	 * 是否为IOS
	 * @return {Boolean} 布尔
	 * @example typeUtils.isIos()
	 */
	isIos: function() {
		var u = navigator.userAgent;
		if (u.indexOf('Android') > -1 || u.indexOf('Linux') > -1) { //安卓手机
			// return "Android";
			return false
		} else if (u.indexOf('iPhone') > -1) {
			// return "iPhone";
			return true
		} else if (u.indexOf('iPad') > -1) {
			// return "iPad";
			return false
		} else if (u.indexOf('Windows Phone') > -1) {
			// return "Windows Phone";
			return false
		} else {
			return false
		}
	},

	/**
	 * 是否为PC
	 * @return {Boolean} 布尔
	 * @example typeUtils.isPC()
	 */
	isPC: function() {
		var userAgentInfo = navigator.userAgent;
		var Agents = ["Android", "iPhone",
			"SymbianOS", "Windows Phone",
			"iPad", "iPod"
		];
		var flag = true;
		for (var v = 0; v < Agents.length; v++) {
			if (userAgentInfo.indexOf(Agents[v]) > 0) {
				flag = false;
				break;
			}
		}
		return flag;
	},

	/**
	 * 获得浏览器类型
	 * @return {String} 字符串
	 * @example typeUtils.browserType()
	 */
	browserType: function() {
		var userAgent = navigator.userAgent;
		var isOpera = userAgent.indexOf("Opera") > -1;
		var isIE = userAgent.indexOf("compatible") > -1 && userAgent.indexOf("MSIE") > -1 && !isOpera;
		var isIE11 = userAgent.indexOf('Trident') > -1 && userAgent.indexOf("rv:11.0") > -1;
		var isEdge = userAgent.indexOf("Edge") > -1 && !isIE;
		var isFF = userAgent.indexOf("Firefox") > -1;
		var isSafari = userAgent.indexOf("Safari") > -1 && userAgent.indexOf("Chrome") == -1;
		var isChrome = userAgent.indexOf("Chrome") > -1 && userAgent.indexOf("Safari") > -1;

		if (isIE) {
			var reIE = new RegExp("MSIE (\\d+\\.\\d+);");
			reIE.test(userAgent);
			var fIEVersion = parseFloat(RegExp["$1"]);
			if (fIEVersion == 7) return "IE7"
			else if (fIEVersion == 8) return "IE8";
			else if (fIEVersion == 9) return "IE9";
			else if (fIEVersion == 10) return "IE10";
			else return "IE7以下"
		}
		if (isIE11) return 'IE11';
		if (isEdge) return "Edge";
		if (isFF) return "FF";
		if (isOpera) return "Opera";
		if (isSafari) return "Safari";
		if (isChrome) return "Chrome";
	},

	/**
	 * 字符串类型检测,是否为手机号,身份证,密码,邮政编码等
	 * @param  {str} 要判断的字符串
	 * @param  {type} 类型
	 * @return {Boolean} 布尔
	 * @example typeUtils.checkStr('10086','tel')
	 */
	checkStr: function(str, type) {
		switch (type) {
			case 'phone': //手机号码
				return /^1[3|4|5|6|7|8|9][0-9]{9}$/.test(str);
			case 'tel': //座机
				return /^(0\d{2,3}-\d{7,8})(-\d{1,4})?$/.test(str);
			case 'card': //身份证
				return /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(str);
			case 'pwd': //密码以字母开头，长度在6~18之间，只能包含字母、数字和下划线
				return /^[a-zA-Z]\w{5,17}$/.test(str)
			case 'postal': //邮政编码
				return /[1-9]\d{5}(?!\d)/.test(str);
			case 'QQ': //QQ号
				return /^[1-9][0-9]{4,9}$/.test(str);
			case 'email': //邮箱
				return /^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/.test(str);
			case 'money': //金额(小数点2位)
				return /^\d*(?:\.\d{0,2})?$/.test(str);
			case 'URL': //网址
				return /(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?/.test(str)
			case 'IP': //IP
				return /((?:(?:25[0-5]|2[0-4]\\d|[01]?\\d?\\d)\\.){3}(?:25[0-5]|2[0-4]\\d|[01]?\\d?\\d))/.test(str);
			case 'date': //日期时间
				return /^(\d{4})\-(\d{2})\-(\d{2}) (\d{2})(?:\:\d{2}|:(\d{2}):(\d{2}))$/.test(str) ||
					/^(\d{4})\-(\d{2})\-(\d{2})$/.test(str)
			case 'number': //数字
				return /^[0-9]$/.test(str);
			case 'english': //英文
				return /^[a-zA-Z]+$/.test(str);
			case 'chinese': //中文
				return /^[\u4E00-\u9FA5]+$/.test(str);
			case 'lower': //小写
				return /^[a-z]+$/.test(str);
			case 'upper': //大写
				return /^[A-Z]+$/.test(str);
			case 'HTML': //HTML标记
				return /<("[^"]*"|'[^']*'|[^'">])*>/.test(str);
			default:
				return true;
		}
	},

	/**
	 * 严格的身份证校验
	 * @param  {sId} 要判断的身份证号
	 * @return {Boolean} 布尔
	 * @example typeUtils.isCardID('45455115415125452154')
	 */
	isCardID: function(sId) {
		if (!/(^\d{15}$)|(^\d{17}(\d|X|x)$)/.test(sId)) {
			alert('你输入的身份证长度或格式错误')
			return false
		}
		//身份证城市
		var aCity = {
			11: "北京",
			12: "天津",
			13: "河北",
			14: "山西",
			15: "内蒙古",
			21: "辽宁",
			22: "吉林",
			23: "黑龙江",
			31: "上海",
			32: "江苏",
			33: "浙江",
			34: "安徽",
			35: "福建",
			36: "江西",
			37: "山东",
			41: "河南",
			42: "湖北",
			43: "湖南",
			44: "广东",
			45: "广西",
			46: "海南",
			50: "重庆",
			51: "四川",
			52: "贵州",
			53: "云南",
			54: "西藏",
			61: "陕西",
			62: "甘肃",
			63: "青海",
			64: "宁夏",
			65: "新疆",
			71: "台湾",
			81: "香港",
			82: "澳门",
			91: "国外"
		};
		if (!aCity[parseInt(sId.substr(0, 2))]) {
			alert('你的身份证地区非法')
			return false
		}
		// 出生日期验证
		var sBirthday = (sId.substr(6, 4) + "-" + Number(sId.substr(10, 2)) + "-" + Number(sId.substr(12, 2))).replace(
				/-/g, "/"),
			d = new Date(sBirthday)
		if (sBirthday != (d.getFullYear() + "/" + (d.getMonth() + 1) + "/" + d.getDate())) {
			alert('身份证上的出生日期非法')
			return false
		}
		// 身份证号码校验
		var sum = 0,
			weights = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2],
			codes = "10X98765432"
		for (var i = 0; i < sId.length - 1; i++) {
			sum += sId[i] * weights[i];
		}
		var last = codes[sum % 11]; //计算出来的最后一位身份证号码
		if (sId[sId.length - 1] != last) {
			alert('你输入的身份证号非法')
			return false
		}
		return true
	}
}
